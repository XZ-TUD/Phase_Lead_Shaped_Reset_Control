...
%     CgLp_PID_sys_freqs2(hn,:) = func_calcr_Cs(1/(s/wr_alpha*2.2+1), s/s, -0.5, n, Ws) .* Lead_freqs .* sys_freqs.*PID2_freqs;
    % Improved steady-state responses
    Shaped_CgLp_PID_sys_freqs(hn,:) = func_calcr_Cs(1/(s/wr_alpha*1.5+1)*1.48, Cs, 0.08, n, Ws) .* C_alpha_P_freqs(hn,:);
%      Shaped_CgLp_PID_sys_freqs(hn,:) = func_calcr_Cs(1/(s/wr_alpha*1.5+1)*1.7, Cs, 0.12, n, Ws) .* Lead_freqs .* sys_freqs.*PID2_freqs;
 
%     Shaped_CgLp_PID_sys_freqs(hn,:) = func_calcr_Cs(1/(s/wr_alpha*1.5+1)*1.34, Cs, 0, n, Ws) .* Lead_freqs .* sys_freqs.*PID2_freqs;
    Cs_Shaped_CgLp_PID_sys_freqs(hn,:) =  Cs_freqs.* Shaped_CgLp_PID_sys_freqs(hn,:);
    
    % Improved transient responses
%     Shaped_CgLp_PID_sys_freqs(hn,:) = func_calcr_Cs(1/(s/wr_alpha*0.75+1)*1, Cs, 0, n, Ws) .* Lead_freqs .* sys_freqs.*PID2_freqs;
...
h = figure; 
subplot(3,1,1);
semilogx(Ws, mag2db(abs(PID_sys_freqs)),'LineWidth',2,'Color','#0072bd'); hold on;
semilogx(Ws, mag2db(abs(CgLp_PID_sys_freqs(1,:))),'LineWidth',2,'Color','#edb120'); hold on;
% semilogx(Ws, mag2db(abs(CgLp_PID_sys_freqs2(1,:))),'LineWidth',2,'Color','b'); hold on;
semilogx(Ws, mag2db(abs(Shaped_CgLp_PID_sys_freqs(1,:))),'LineWidth',2,'Color','#d95319'); hold on;
semilogx(Ws, mag2db(abs(Cs_Shaped_CgLp_PID_sys_freqs(1,:))),'LineWidth',2,'Color','#999999'); hold on;
grid on;
ylabel('Magnitude [dB]');
set(gca,'fontsize', 16); 
...
subplot(3,1,2);
semilogx(Ws, mag2db(abs(CgLp_PID_sys_freqs(2,:))),'--','LineWidth',2,'Color','#edb120'); hold on;
semilogx(Ws, mag2db(abs(Shaped_CgLp_PID_sys_freqs(2,:))),'--','LineWidth',2,'Color','#d95319'); hold on;
grid on;
ylabel('Magnitude [dB]');
set(gca,'fontsize', 16); 
set(gca,'FontName','Times New Roman','fontSize', 16);
...
subplot(3,1,3);
semilogx(Ws, rad2deg(angle(PID_sys_freqs)),'LineWidth',2,'Color','#0072bd'); hold on;
semilogx(Ws, rad2deg(angle(CgLp_PID_sys_freqs(1,:))),'LineWidth',2,'Color','#edb120'); hold on;
% semilogx(Ws, rad2deg(angle(CgLp_PID_sys_freqs2(1,:))),'LineWidth',2,'Color','b'); hold on;
semilogx(Ws, rad2deg(angle(Shaped_CgLp_PID_sys_freqs(1,:))),'LineWidth',2,'Color','#d95319'); hold on;
...
h = figure; 
semilogx(Ws, (abs(CgLp_PID_sys_freqs(2,:)))./(abs(CgLp_PID_sys_freqs(1,:))),'LineWidth',2,'Color','#edb120'); hold on;
semilogx(Ws, (abs(Shaped_CgLp_PID_sys_freqs(2,:)))./ (abs(Shaped_CgLp_PID_sys_freqs(1,:))),'LineWidth',2,'Color','#d95319'); hold on;
% semilogx(Ws, mag2db(abs(Cs_Shaped_CgLp_PID_sys_freqs(1,:))),'LineWidth',2,'Color','#999999'); hold on;
grid on;
ylabel('Magnitude [dB]');
set(gca,'fontsize', 16); 
