...
for hn = 1:Nh
    n = 2*hn-1;
    CgLp_PID_sys_freqs(hn,:) = hosidfcalc(ss(A,B,C,D), gamma, n, Ws) .* Lead_freqs .* sys_freqs.*PID2_freqs;
%     CgLp_PID_sys_freqs2(hn,:) = func_calcr_Cs(1/(s/wr_alpha*2.2+1), s/s, -0.5, n, Ws) .* Lead_freqs .* sys_freqs.*PID2_freqs;
    % Improved steady-state responses
    Shaped_CgLp_PID_sys_freqs(hn,:) = func_calcr_Cs(1/(s/wr_alpha*1+1)*1, Cs, 0, n, Ws) .* Lead_freqs .* sys_freqs.*PID2_freqs;
   
%     Shaped_CgLp_PID_sys_freqs(hn,:) = func_calcr_Cs(1/(s/wr_alpha*1.5+1)*1.34, Cs, 0, n, Ws) .* Lead_freqs .* sys_freqs.*PID2_freqs;
    Cs_Shaped_CgLp_PID_sys_freqs(hn,:) =  Cs_freqs.*func_calcr_Cs(1/(s/wr_alpha*1+1)*1, Cs, 0, n, Ws) .* Lead_freqs .* sys_freqs.*PID2_freqs;
...
h = figure; 
subplot(2,1,1);
semilogx(freqs, mag2db(abs(PID_sys_freqs)),'LineWidth',2,'Color','#0072bd'); hold on;
semilogx(freqs, mag2db(abs(CgLp_PID_sys_freqs(1,:))),'LineWidth',2,'Color','#edb120'); hold on;
...
% 
% 
% semilogx(freqs, mag2db(abs(CgLp_PID_sys_freqs(2,:))),'--','LineWidth',2,'Color','#edb120'); hold on;
...
subplot(2,1,2);
semilogx(freqs, rad2deg(angle(PID_sys_freqs)),'LineWidth',2,'Color','#0072bd'); hold on;
semilogx(freqs, rad2deg(angle(CgLp_PID_sys_freqs(1,:))),'LineWidth',2,'Color','#edb120'); hold on;
